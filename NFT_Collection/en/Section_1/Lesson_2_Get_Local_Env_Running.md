...
1. `mkdir epic-nfts` will create a directory named "epic-nfts".
2. `cd epic-nfts` is used to enter the newly created directory.
3. `npm init -y` will simply generate an empty npm project without going through an interactive process. The -y stands for yes.
4. `npm install --save-dev hardhat` will install Hardhat.
